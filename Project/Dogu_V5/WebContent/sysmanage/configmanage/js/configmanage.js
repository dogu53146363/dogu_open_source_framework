...
function getConfInfo(){
	...
	layui.use(['layer', 'table', 'element'], function(){
		...
		table.render({
			elem: '#dataTable'
			,height: 'full-70'
			,url: rootPath + '/configmanage/getConfInfo' //数据接口
			,method: "post"
			,where: {
...
function sureAddConf(){
	...
	if($.trim(confkey) == "" || confkey == null){
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/configmanage/addConf',
			beforeSend: function () {
		        showLoad();
		    },
			data : {
...
function changeConf(confkey){
	...
	$.ajax({
		type : 'post',
		url : rootPath + '/configmanage/getOneConfInfo',
		beforeSend: function () {
			showLoad();
	    },
		data : {
...
function sureChangeConf(){
	...
	if($.trim(confkey) == "" || confkey == null){
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/configmanage/updateConf',
			beforeSend: function () {
		        showLoad();
		    },
			data : {
...
function delConf(confkey){
	parent.layer.confirm('确定删除配置信息？', {
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/configmanage/deleteConf',
			beforeSend: function () {
		        showLoad();
		    },
			data : {
