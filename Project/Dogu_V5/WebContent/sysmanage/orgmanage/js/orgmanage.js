...
function getOrgZtreeInfo(){
	...
	var setting = {
		async : {
			enable : true,
			type : 'post',
			url : rootPath + '/orgmanage/getOrgTreeInfo',
			otherParam: []
		},
		check : {
			enable : true,
...
function initOrgTable(){
	...
	layui.use(['layer', 'table', 'element'], function(){
		...
		table.render({
			elem: '#dataTable'
			,height: 'full-80'
			,url: rootPath + '/orgmanage/getOrgTable' //数据接口
			,method: "post"
			,where: {
				orgID : queryOrgId
			}
...
function initChangeOrgInfoTable(orgId){
	$.ajax({
		type : 'post',
		url : rootPath + '/orgmanage/getOneOrgInfo',
		beforeSend: function () {
			showLoad();
		},
		data : {
		...
		success : function(data) {
			if(data.code == "0"){
				var UseStatus = "";
				if(data.use == "Y"){
					UseStatus = "checked='checked'";
				}
				var changeOrgContext = 
					"<table class='layui-table'>"+
					"<tr>"+
					"<td style='text-align: right;'>组织机构编号：</td>"+
					"<td><input id='orgId' class='layui-input' disabled='disabled' value='"+data.orgId+"' placeholder='*请输入组织机构编号' type='text' style='width: 250px;'/></td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: right;'>组织机构名称：</td>"+
					"<td><input id='orgName' class='layui-input' placeholder='*请输入组织机构名称' value='"+data.orgName+"' type='text' style='width: 250px;'/></td>"+
					"</tr>"+
					"<td style='text-align: right;'>所属组织机构编号：</td>"+
					"<td><input id='pOrgId' class='layui-input' placeholder='*请输入所属组织机构编号' value='"+data.orgPId+"' type='text' style='width: 250px;'/></td>"+
					"</tr>"+
					"<td style='text-align: right;'>备注：</td>"+
					"<td><input id='Note' class='layui-input' placeholder='*请输入备注' type='text' value='"+data.note+"' style='width: 250px;'/></td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: right;'>是否启用：</td>"+
					"<td>"+
					"&nbsp;<input id='Use' "+UseStatus+" type='checkbox'/>"+
					"</td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: center;' colspan='2'>"+
...
function sureChangeOrg(){
	...
	if($.trim(orgId) == "" || orgId == null){
		...
		parent.layer.confirm('注意,添加的此组织机构状态为不启用！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/orgmanage/updateOrg',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function titleDelete(){
	...
	if(checkedNode.length < 1){
		...
		parent.layer.confirm('确认删除该节点吗？如果删除的节点下包含子节点，则相应的子节点也会被删除！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/orgmanage/beforeDelOrgQuery',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function sureDeleteOrg(deleteOrgId){
	$.ajax({
		type : 'post',
		url : rootPath + '/orgmanage/deleteOrg',
		beforeSend: function () {
			showLoad();
		},
		data : {
