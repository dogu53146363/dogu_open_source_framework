...
public class SysHandler extends Handler {
	...
	public void handle(String target, HttpServletRequest request,
		...
		if (path.endsWith(Constant.DOJSP) || path.endsWith(Constant.DOHTM)
				|| path.endsWith(Constant.DOHTML) || path.endsWith(Constant.DOSHTM)
				|| path.endsWith(Constant.DOSHTML)) {
					...
					Record rec = Db
							.findFirst(
									"SELECT COUNT(1) AS NUM FROM SYS_MENU_LIMIT MENU_LIMIT,SYS_MENU_DCT MENU_DCT"
											...
											+ " AND MENU_DCT.F_PATH = ? AND MENU_LIMIT.F_MENUID = MENU_DCT.F_MENUID AND MENU_DCT.F_USE = ? ",
