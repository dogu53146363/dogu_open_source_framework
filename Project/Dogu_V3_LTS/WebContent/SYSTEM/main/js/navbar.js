/** navbar.js By Dogu E_mail:lzq_jn@qq.com HomePage:http://www.dogu.site */
...
layui.define(['element'], function (exports) {
    ...
    Navbar.prototype.render = function () {
        ...
        if (typeof (_config.elem) != 'string' && typeof (_config.elem) != 'object') {
            layer.alert("Navbar error: elem参数未定义或设置出错，具体设置格式请参考文档API.",{icon: 7});
        }
        var $container;
        ...
        if ($container.length == 0) {
            layer.alert("Navbar error:找不到elem参数配置的容器，请检查.",{icon: 7});
        }
        if (_config.data == undefined && _config.url == undefined) {
            layer.alert("Navbar error:请为Navbar配置数据源.",{icon: 7});
        }
                ...
                if (cacheNavbar.navbar == undefined) {
                    $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            layer.alert('Navbar error:' + error,{icon: 7});
                        },
    ...
    Navbar.prototype.on = function (events, callback) {
        ...
        if (typeof (events) != 'string') {
            layer.alert('Navbar error:事件名配置出错，请参考API文档.',{icon: 7});
        }
