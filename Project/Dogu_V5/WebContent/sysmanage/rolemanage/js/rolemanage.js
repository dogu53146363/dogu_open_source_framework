...
function getRoleInfo(){
	...
	layui.use(['layer', 'table', 'element'], function(){
		...
		table.render({
			elem: '#dataTable'
			,height: 'full-70'
			,url: rootPath + '/rolemanage/getRoleInfo' //数据接口
			,method: "post"
			,where: {
				use : use
			}
...
function sureAddRole(){
	...
	if($.trim(roleId) == "" || roleId == null){
		...
		parent.layer.confirm('注意,添加的此角色将不会被启用！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/rolemanage/addRole',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function ChangeRole(roleID){
	$.ajax({
		type : 'post',
		url : rootPath + '/rolemanage/getOneRoleInfo',
		beforeSend: function () {
			showLoad();
		},
		data : {
		...
		success : function(data) {
			if(data.code == "0"){
				...
				var changeRoleContext = 
					...
					"<tr>"+
					"<td style='text-align: right;'>角色编号：</td>"+
					"<td><input id='roleid' class='layui-input' disabled='disabled' placeholder='*请输入角色编号' value='"+data.roleid+"' type='text' style='width: 250px;'/></td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: right;'>角色名：</td>"+
					"<td><input id='rolename' class='layui-input' placeholder='*请输入角色名' value='"+data.rolename+"' type='text' style='width: 250px;'/></td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: right;'>是否启用：</td>"+
					"<td>"+
					"<input id='use' "+UseChecked+" type='checkbox'/>"+
					"</td>"+
					"</tr>"+
					"<tr>"+
					"<td style='text-align: center;' colspan='2'>"+
...
function sureChangeRole(){
	...
	if($.trim(roleid) == "" || roleid == null){
		...
		parent.layer.confirm('注意,此角色将不会被启用！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/rolemanage/updateRole',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function delRole(roleid){
	parent.layer.confirm('确定删除角色？', {
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/rolemanage/beforeDelRoleQuery',
			beforeSend: function () {
				showLoad();
			},
			data : {
...
function suerDeleteRole(roleid){
	$.ajax({
		type : 'post',
		url : rootPath + '/rolemanage/deleteRole',
		beforeSend: function () {
			showLoad();
		},
		data : {
