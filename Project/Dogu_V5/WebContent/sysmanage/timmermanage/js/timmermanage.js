...
function gettimmerInfo(){
	...
	layui.use(['layer', 'table', 'element'], function(){
		...
		table.render({
			elem: '#dataTable'
			,height: 'full-70'
			,url: rootPath + '/timmermanage/getTimmerInfo' //数据接口
			,method: "post"
			,where: {
				zt : zt
			}
...
function Start(timmerid){
	$.ajax({
		type : 'post',
		url : rootPath + '/timmermanage/startTimmer',
		beforeSend: function () {
			showLoad();
		},
		data : {
...
function Stop(timmerid){
	$.ajax({
		type : 'post',
		url : rootPath + '/timmermanage/stopTimmer',
		beforeSend: function () {
			showLoad();
		},
		data : {
...
function Delete(timmerid){
	parent.layer.confirm('确定删除定时任务？删除之后定时任务将不能使用！', {
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/timmermanage/deleteTimmer',
			beforeSend: function () {
				showLoad();
			},
			data : {
...
function sureAddTimmer(){
	...
	if($.trim(timmerid) == "" || timmerid == null){
		...
		parent.layer.confirm('注意,添加的定时任务将不会被启用！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/timmermanage/addTimmer',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function changeTimmer(timmerid){
	...
	$.ajax({
		type : 'post',
		url : rootPath + '/timmermanage/getOneTimmerInfo',
		beforeSend: function () {
			showLoad();
		},
		data : {
...
function sureChangeTimmer(timmerid){
	...
	if($.trim(timmerclz) == "" || timmerclz == null){
		...
		parent.layer.confirm('注意,添加的定时任务将不会被启用！', {
			...
			$.ajax({
				type : 'post',
				url : rootPath + '/timmermanage/updateTimmer',
				beforeSend: function () {
					showLoad();
				},
				data : {
...
function showModelessDialog(){
	var url = rootPath+"/timmermanage/cronPage";//转向网页的地址; 
