...
layui.define(['layer', 'element', 'admin', 'contextMenu'], function (exports) {
    ...
    var IsFirstOpen = false;//是否第一次打开该tab页
    var homeUrl;  // 主页地址，主页不参与Tab记忆
	var needRefreshPage = ["fileupload.jsp","B.jsp","C.jsp","D.jsp"];//切换时打开"页面刷新"功能需要刷新的页面
    ...
    element.on('tab(' + tabFilter + ')', function (data) {
		...
		var pagePath = this.getAttribute("lay-id");
		var allowFresh = 0;
        if(needRefreshPage.length > 0 && needRefreshPage[0] != ""){
        	for(var i=0;i<needRefreshPage.length;i++){
		        if(pagePath.indexOf(needRefreshPage[i]) > -1){
		        	allowFresh++;
		        }
        	}
        }
        var autoRefresh = $(tabDOM).attr('lay-autoRefresh');
        if (autoRefresh === 'true' && allowFresh > 0 && !IsFirstOpen) {// 第一次打开Tab不刷新
