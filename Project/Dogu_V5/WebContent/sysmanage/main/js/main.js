...
function initMenu(){
	layui.config({
        ...
        // ajax渲染侧边栏
        $.get(rootPath + '/main/getMenuInfo', function (res) {
        	if(res.code == 200){
        		...
        		laytpl(sideNav).render(res.data, function (html) {
	                ...
	                element.render('nav');
	                admin.activeNav(rootPath+'/main/desktop');//默认首页
	            });
	            //查询"我的桌面"路径
	            $.ajax({
					type : 'post',
					url : rootPath + '/main/getDeskTopPath',
...
function getUserInfo(){
	$.ajax({
		type : 'post',
		url : rootPath + '/main/getIndexChangeUserInfo',
		success : function(data) {
			initUserInfo(data);
		},
		error : function(XMLHttpRequest, textStatus, errorThrown) {
...
function sureChangeInfo(){
	...
	if($.trim(name)==""||name==null){
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/main/indexChangeUser',
			data : {
				name : name,
				sex: sex,
				tel : tel,
...
function sureChangePsw(){
	...
	if(newpassword != againpassword){
		...
		$.ajax({
			type : 'post',
			url : rootPath + '/main/validatePassword',
			data : {
