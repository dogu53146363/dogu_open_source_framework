/** tab.js By Dogu E_mail:lzq_jn@qq.com HomePage:http://www.dogu.site */
...
layui.define(['element'], function (exports) {
    ...
    Tab.prototype.init = function () {
        ...
        if (typeof (_config.elem) != 'string' && typeof (_config.elem) != 'object') {
            layer.alert("Tab error: elem参数未定义或设置出错，具体设置格式请参考文档API.",{icon: 7});
        }
        var $container;
        ...
        if ($container.length == 0) {
            layer.alert("Tab error:找不到elem参数配置的容器，请检查.",{icon: 7});
        }
        var filter = $container.attr('lay-filter');
        if (filter == undefined || filter == '') {
        	layer.alert("Tab error:请为elem容器设置一个lay-filter过滤器.",{icon: 7});
        }
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex == -1) {
            ...
            if (_config.maxSetting != 'undefined') {
                ...
                if (typeof _config.maxSetting.max == 'number') {
                    if (currentTabCount == _config.maxSetting.max) {
                        layer.msg(_config.maxSetting.tipMsg);
            ...
            globalTabIdIndex++;
            var content = '<iframe class="iframe_content" src="' + data.href + '" data-id="' + globalTabIdIndex + '"></iframe>';
            var title = '';
...
//                $(this).height(ELEM.contentBox.height()-5);
        ...
        if (_config.showContextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                //判断，如果存在右键菜单的div，则移除，保证页面上只存在一个
                ...
                ul += '<li data-target="closeOther" title="关闭其他选项卡"><i class="fa fa-times-circle" aria-hidden="true"></i> 关闭其他</li>';
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                ...
                                case 'closeCurrent': //关闭当前
                                	if(clickIndex == "MAINPAGE"){
                                		parent.layer.alert("该页面不可以关闭！",{icon: 7});
